...
function validarSenha() {
    ...
    if (senha.length < 8) {
        senhaError.textContent = "A senha deve possuir pelo menos 8 caracteres.";
        senhaInput.classList.add("input-error");
        return false;
    }
...
loginForm.addEventListener("submit", async function (event) {
    ...
    if (!emailValido || !senhaValida) {
        mostrarMensagem("Verifique os dados informados.", "error");
        return;
    }
    ...
    try {
        // Requisição real para o Spring Boot usando a função importada
        ...
        if (response && response.status === 429) {
            mostrarMensagem("Muitas tentativas de login. Tente novamente em 1 minuto.", "error");
            return;
        }
        ...
        if (!dados || !dados.token) {
            mostrarMensagem("Resposta inesperada do servidor. Tente novamente.", "error");
            return;
        }
        ...
        mostrarMensagem("Não foi possível conectar ao servidor. Verifique se a API está no ar.", "error");
    } finally {
